...
export async function saveNoteAction(repositorySlug: string, noteId: string, formData: FormData) {
  ...
  const editableByClearanceValue = formData.get("editable_by_clearance");
  ...
  await updateNote(repositorySlug, noteId, {
    title,
    content_markdown: contentMarkdown || contentText,
    content_text: contentText,
    content_json: contentJson || undefined,
    ...(editableByClearanceValue === null
      ? {}
      : { editable_by_clearance: String(editableByClearanceValue) === "true" }),
  });
